...
void simulation(bool affiche, int rank,int nbp)
{
    ...
    int flag;
    if (rank >0){
        MPI_Irecv(&zero_quit,1,MPI_C_BOOL,0,1,MPI_COMM_WORLD,& quitRequest );
    ...
    while (!quitting)
    {
        ...
        if (rank == 0){
            ...
            for ( const auto& e : events)
            {
                if (e->kind_of_event() == sdl2::event::quit){
                    ...
                    zero_quit = true;
                    for (int k=1;k<nbp;k++){
                        MPI_Isend(&zero_quit, 1, MPI_C_BOOL, k, 1, MPI_COMM_WORLD,& quitRequest ) ;
            ...
            MPI_Recv(temp_grid.data(), 3*largeur_grille*hauteur_grille, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD ,& status );
        ...
        if(rank > 0){
            ...
            MPI_Send(temp_grid.data(), 3*largeur_grille*hauteur_grille, MPI_INT, 0, 0, MPI_COMM_WORLD);
        ...
        if (rank == 0){
            ...
            std::cout << "rank "<<rank<<". Temps calcul par etape (recuperation + affichage ): " << elapsed_seconds2.count() 
                  << std::endl;
        }else{
            end = std::chrono::system_clock::now();
            std::chrono::duration<double> elapsed_seconds2 = end-start;
            std::cout << "rank "<<rank<<". Temps calcul par etape (calcul + envoi ): " << elapsed_seconds2.count() 
                  << std::endl;
        }

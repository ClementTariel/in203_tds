...
int main(int nargs, char *argv[] ) 
 { 
    ...
    const int num_row_per_block = 10;
   ...
   //maitre-esclave
    if ( rank == 0 ) // rank == 0 => master
    {
        ...
        for ( int i = 1 ; i < nbp ; ++i ) {
            ...
            count_task += num_row_per_block;
        }
        while ( count_task < H) {
            // s t a t u s c o n t i e n d r a l e numéro du proc ayant envoyé
            // l e r é s u l t a t . . . : s t a t u s .MPI_SOURCE en MPI
            std::vector<int> pixels_temp(num_row_per_block*W+1);
            MPI_Recv(&pixels_temp[0] , num_row_per_block*W+1 , MPI_INT , MPI_ANY_SOURCE, 0, MPI_COMM_WORLD ,& status );
            for (int k = 0; k < num_row_per_block*W ; k++){
                pixels[W*pixels_temp[num_row_per_block*W]+k] = pixels_temp[k];
            }
        ...
        for ( int i = 1 ; i <nbp ; ++i ){
        	std::vector<int> pixels_temp(num_row_per_block*W+1);
            MPI_Recv(&pixels_temp[0] , num_row_per_block*W+1 , MPI_INT , i, 0, MPI_COMM_WORLD ,& status );
            for (int k = 0; k < num_row_per_block*W ; k++){
                pixels[W*pixels_temp[num_row_per_block*W]+k] = pixels_temp[k];
            }
            MPI_Send(&count_task, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
        ...
        while (num_task >= 0){
            ...
            if (num_task >= 0) {
                ...
                std::vector<int> pixels(num_row_per_block*W+1);
                for (int k=0;k<num_row_per_block;k++){
	                computeMandelbrotSetRow(W, H, maxIter, num_ligne, &pixels[k*W]);
	            }
                pixels[num_row_per_block*W] = num_task;
                //e x e c u t e _ t a s k ( num_task , . . . ) ;
                // Renvoie l e r é s u l t a t avec son numéro
                MPI_Send(&pixels[0], num_row_per_block*W+1, MPI_INT, 0, 0, MPI_COMM_WORLD);
                //send ( r e s u l t , . . . , 0 , . . . ) ;
            }
        }

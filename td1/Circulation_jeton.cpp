...
int main( int nargs, char* argv[] )
{
	...
	if(rank==0){
		//myval = 12;
		std::cout << "Valeur du jeton : " << myval << " ( je suis le processus n°" << rank << ".)\n";
		MPI_Send (&myval , 1, MPI_INT , rank +1, 0, MPI_COMM_WORLD );
		MPI_Recv (&myval , 1, MPI_INT , nbp -1, 0, MPI_COMM_WORLD ,& status );
		myval++;
		std::cout << "Valeur du jeton : " << myval << " ( je suis le processus n°" << rank << ".)\n";
	}else if(rank==nbp -1){
		MPI_Send (&myval , 1, MPI_INT , 0, 0, MPI_COMM_WORLD );
		MPI_Recv (&myval , 1, MPI_INT , rank -1, 0, MPI_COMM_WORLD ,& status );
		myval++;
		std::cout << "Valeur du jeton : " << myval << " ( je suis le processus n°" << rank << ".)\n";
		//MPI_Send (&myval , 1, MPI_INT , 0, 0, MPI_COMM_WORLD );
	}else{
